...
class TwoPhaseVelProjJump2d:
    def __init__(self, res, dx, phi, vel, rho_ratio, base_level=3, real=float):
        self.res = res
        self.dx = dx
        self.phi = phi
        self.vel = vel
        ...
        self.jump = [ti.field(dtype=real, shape = (res[0] + 1, res[1])), ti.field(dtype=real, shape = (res[0], res[1] + 1))]
        self.solver = AMGPCG_2D(res, base_level, real)

    def project(self, max_iters=-1, verbose=False, rel_tol=1e-12, abs_tol=1e-14, eps=1e-20):
        self.set_bc_kernel()
        self.calc_div_kernel(self.solver.b)
        self.add_jump_to_rhs_kernel(self.solver.b)
        self.set_lhs_kernel(
    ...
    @ti.kernel
    def set_lhs_kernel(
        self, is_dof: ti.template(), Adiag: ti.template(), Ax: ti.template()
    ):
        for i, j in is_dof:
            is_dof[i, j] = True
        ...
        for i, j in self.vel[0]:
            if i > 0 and i < self.res[0]:
                if self.phi[i, j] < 0.0 and self.phi[i - 1, j] < 0.0:
                    p0 = pressure[i - 1, j]
                    p1 = pressure[i, j]
                    self.vel[0][i, j] += (p1 - p0) * Ax[i - 1, j][0]
        ...
        for i, j in self.vel[1]:
            if j > 0 and j < self.res[1]:
                if self.phi[i, j] < 0.0 and self.phi[i, j - 1] < 0.0:
                    p0 = pressure[i, j - 1]
                    p1 = pressure[i, j]
                    self.vel[1][i, j] += (p1 - p0) * Ax[i, j - 1][1]
